...
/**
 * Custom TypeScript wrapper around keycloak-js
...
export class KeycloakWrapper {
  ...
  async init(initOptions: Keycloak.KeycloakInitOptions = {}): Promise<boolean> {
    ...
    if (this.initialized) {
      console.log('🔐 Keycloak already initialized, returning authenticated state');
  ...
  private async performInitialization(initOptions: Keycloak.KeycloakInitOptions): Promise<boolean> {
    try {
      ...
      throw new Error(`Keycloak initialization failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
